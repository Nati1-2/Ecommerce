...
// Known valid demo credentials for offline fallback mode
const DEMO_ACCOUNTS = {
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. If it's a demo account, check password locally first to ensure instant offline access
    ...
    try {
      ...
      // If backend returned explicit error (like invalid password), throw error immediately
      if (data.error) {
        throw new Error(data.error);
      }
    } catch (err: any) {
      // Check if error is explicit password mismatch
      ...
      // Offline mode fallback: strictly check DEMO_ACCOUNTS with EXACT password match
      const demoAccount = DEMO_ACCOUNTS[normalizedEmail as keyof typeof DEMO_ACCOUNTS];
      if (demoAccount) {
        if (demoAccount.password !== password) {
          setError("Invalid email or password. Please try again.");
