...
export default function VendorExportModal() {
  const { isExportModalOpen, setIsExportModalOpen } = useAdminVendorStore();
  ...
  const handleExport = () => {
    showToast(`Vendor directory exported as ${format} (${segment})`, "success");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-base">
                Export Marketplace Vendors
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">Download vendor registry & payouts report</p>
            </div>
          </div>
        ...
        <div className="space-y-4 text-xs">
          ...
          <div>
            <label className="font-bold text-slate-700 dark:text-slate-300">Target Segment</label>
            <select
              value={segment}
            ...
            >
              <option value="All Vendors">All Marketplace Vendors (12,500)</option>
              <option value="Active Approved Stores">Active Approved Stores Only</option>
              <option value="Pending Applications">Pending Onboarding Applications</option>
              <option value="Suspended Stores">Suspended Stores Only</option>
            </select>
          </div>
        </div>
