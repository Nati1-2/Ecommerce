...
export default function ReviewExportModal() {
  const { isExportModalOpen, setIsExportModalOpen } = useAdminReviewStore();
  const showToast = useAdminDashboardStore((state) => state.showToast);
  const [format, setFormat] = useState<"CSV" | "Excel" | "PDF">("CSV");
  const [segment, setSegment] = useState("All Reviews");

  if (!isExportModalOpen) return null;

  const handleExport = () => {
    showToast(`Review dataset exported as ${format} (${segment})`, "success");
    setIsExportModalOpen(false);
  };

  const formatsList = [
    { name: "CSV" as const, desc: "Comma-Separated Values", icon: FileText },
    { name: "Excel" as const, desc: "Microsoft Excel spreadsheet (.xlsx)", icon: FileSpreadsheet },
    { name: "PDF" as const, desc: "Executive moderation summary report", icon: FileText },
  ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Download moderation audit logs & rating datasets</p>
            </div>
          </div>
        ...
        <div className="space-y-4 text-xs">
          ...
          <div>
            <label className="font-bold text-slate-700 dark:text-slate-300 font-medium">Target Segment</label>
            <select
              value={segment}
            ...
            >
              <option value="All Reviews">All Reviews (5,000,000)</option>
              <option value="Pending Queue">Pending Moderation Queue Only</option>
              <option value="Reported Content">Reported & Flagged Content</option>
              <option value="Removed Violations">Removed Policy Violations</option>
            </select>
          </div>
        </div>
