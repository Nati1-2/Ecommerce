  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-slate-900">Platform Health & Microservices Monitoring</h3>
            <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-full border border-emerald-200/60">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {coreServices.map((cs) => {
          const Icon = cs.icon;
          const isConnected = cs.val === "Connected" || cs.val === "Running" || cs.val === "Healthy" || cs.val === "Operational";

          return (
            <div key={cs.name} className="p-4 bg-slate-50/70 rounded-2xl border border-slate-100/90">
              ...
              <p className="text-xs font-bold text-slate-900 mt-3">{cs.name}</p>
            </div>
          );
        })}
      </div>
