...
export default function ProductExportModal() {
  const { isExportModalOpen, setIsExportModalOpen } = useAdminProductStore();
  const showToast = useAdminDashboardStore((state) => state.showToast);
  const [format, setFormat] = useState<"CSV" | "Excel" | "PDF">("CSV");
  const [segment, setSegment] = useState("All Products");

  if (!isExportModalOpen) return null;

  const handleExport = () => {
    showToast(`Product catalog exported as ${format} (${segment})`, "success");
  ...
  const formatsList = [
    ...
    { name: "PDF" as const, desc: "Executive catalog summary report", icon: FileText },
  ];
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-6">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Download product inventory reports</p>
            </div>
          </div>
        ...
        <div className="space-y-4 text-xs">
          ...
          <div>
            ...
            >
              <option value="All Products">All Catalog Products (850,000)</option>
              <option value="Pending Queue">Pending Approval Queue Only</option>
              <option value="Reported Items">Reported / Flagged Products</option>
              <option value="Approved Published">Approved Published Only</option>
            </select>
          </div>
        </div>
